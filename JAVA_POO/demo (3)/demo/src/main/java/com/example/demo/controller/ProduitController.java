...
public class ProduitController {
    ...
    @GetMapping("/affiche")
    public ModelAndView getListProduit(@CookieValue(value = "couleur") String couleurCookie,HttpServletRequest request){

        ModelAndView modelAndView = new ModelAndView();
        modelAndView.addObject("couleurCookie", couleurCookie);
    ...
    @GetMapping("/couleur")
    public ModelAndView couleur(){
        ModelAndView modelAndView = new ModelAndView();
        modelAndView.setViewName("couleur");
        return modelAndView;
    }

    @PostMapping("/couleur")
    public String mettreCouleur(HttpServletRequest request, HttpServletResponse response){
        String valeur = request.getParameter("couleur");
        Cookie cookie = new Cookie("couleur", valeur);
        cookie.setMaxAge(36000);
        response.addCookie(cookie);
        return "redirect:/product/affiche";
    ...
    public String postFormulaire(@ModelAttribute Produit produit ){
